...
const ScrollContainer = (props) => {
    ...
    const handleLikeClick = async (workId) => {
        ...
        await set(
            firebaseRef(rtdb, `users/${currentUser.uid}/liked`),
            updatedLikes
        );
    ...
    const handleSavedClick = async (workId) => {
        ...
        await set(
            firebaseRef(rtdb, `users/${currentUser.uid}/saved`),
            updatedSaved
        );
                    ...
                    <Item key={el.id || index}>
                        <ItemTextContainer>
                            ...
                            <ItemTitle
                                ...
                                onClick={() => handleReadClick(el.id)}
                            />
                            <ItemText
                                dangerouslySetInnerHTML={{ __html: el.content.slice(3, 300) }}
                            />
                            <ItemGenre>{el.genre}</ItemGenre>
                        </ItemTextContainer>
                        <Reaction>
                            <ReactionIconButton
                                liked={currentLikes.includes(el.id)}
                                onClick={() => {
                            ...
                            <ReactionIconButton
                                saved={currentSaved.includes(el.id)}
                                onClick={() => {
                            ...
                            >
                                ...
                                >
                                    <path
                                        stroke={
                                            currentSaved.includes(el.id) ? "orange" : "currentColor"
                                        }
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="1.3"
